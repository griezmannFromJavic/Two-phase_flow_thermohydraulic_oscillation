...
def vanjska_iteracija_velocity_based(trenutak, pretpostavka, vrijednost_proslog_trenutka, podrelaksacija):
    ...
    brzina = zokg_brzina(pretpostavka[0], brzina_na_ulazu, pretpostavka[1], p_niskotlacno[trenutak], pretpostavka[2],
                         pretpostavka[3], gustoca_na_ulazu, dx_p, podrelaksacija[0])
...
def vanjska_iteracija_piso(trenutak, pretpostavka, vrijednost_proslog_trenutka, podrelaksacija):
...
def rjesavac(pocetni_trenutak, podrelaksacija, rezidual_limit):
    for t in range(pocetni_trenutak, n_t):
        ...
        vremena[t] = vremena[t - 1] + dt
        snimi_trenutak(rjesenje_trenutka, t)
    return print('rjeseno')

def inicijalizacija(podrelaksacija, rezidual_limit):
...
zeljeni_rezidual = 4e-5
# zeljeni_rezidual = 1e-4
...
podrelaksacije = [imp, eksp, imp, eksp, eksp, 1, eksp]

# podrelaksacije[1] = 0.03
# podrelaksacije[3] = 0.03 # 0.002 sigurno radi, možda je to uzrok greškama

# inicijalizacija(podrelaksacije, reziduali)
