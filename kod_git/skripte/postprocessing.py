...
def prikaz_velicina_prostorno(trenutak, velicina):
    ...
    plt.plot(lokacije_sredista, rjesenje[trenutak, velicina, :], label=str(vremena[trenutak]))
    plt.legend()
...
def prikaz_izracunatih_velicina_prostorno(trenutak, velicina):
    plt.plot(lokacije_sredista, rjesenje[trenutak, :], label=str(vremena[trenutak]))
    plt.legend()
    plt.xlabel('udaljenost od ustrujne povrsine [m]')
    plt.ylabel(raspored_velicina[velicina] + ' ' + jedinice[velicina])
    return None
...
def zom_stacionarno_stanje(ulazna_granica, izlazna_granica, trenutak):
    maseni_tok_na_ulazu = (d_u ** 2 * np.pi / 4) * rjesenje[trenutak, 3, ulazna_granica] * rjesenje[trenutak, 0, ulazna_granica]
    maseni_tok_na_izlazu = (d_u ** 2 * np.pi / 4) * rjesenje[trenutak, 3, izlazna_granica] * rjesenje[trenutak, 0, izlazna_granica]
    return maseni_tok_na_izlazu / maseni_tok_na_ulazu

def zoe_stacionarno_stanje(ulazna_granica, izlazna_granica, trenutak):
...
def stacionarnost_stijenke(trenutak):
    ...
    plt.plot(lokacije_sredista, omjer)
    plt.plot([0, l_c], [1, 1])
    return omjer
...
def provjera_alphe(p, x_min, x_max, m, q):
    ...
    omjer = alpha / alpha_0
    plt.plot(np.linspace(x_min, x_max, 1000), alpha, label=str(q))
    # plt.legend()
...
def prikaz_temperatura(trenutak):
    ...
    plt.plot(lokacije_sredista, ts[trenutak], label='temperatura isparavanja')
    # plt.plot([0, l_c], [temperatura_ogrijevnog_medija, temperatura_ogrijevnog_medija], label='temperatura ogrijevnog medija')
    plt.plot(lokacije_sredista, rjesenje[trenutak, 4, :], label='temperatura fluida')
    plt.plot(lokacije_sredista, rjesenje[trenutak, 5, :], label='temperatura stijenke')
    plt.legend()
    return None

def prikaz_x_termo(trenutak):
    xt = termo_udio_pare()[trenutak]
    plt.plot(lokacije_sredista, xt)
    plt.plot([0, l_c], [0, 0])
...
def provjera_dpdl_trenja(p, x_min, x_max, m, epsilon):
    ...
    plt.plot(np.linspace(x_min, x_max, 1000), dpdl, label='cisto')
    # plt.legend()
    return None
...
def prikaz_lokacija_isparavanja():
    ...
    lokacije = [lokacije_sredista[i] for i in elementi]
...
def achard_bezdimenzijske(trenutak):
    ...
    gamma = f * l_c / (2 * d_u)
    fr = v_0 ** 2 / (g * sin_beta * l_c)
    fr_inverz = 1 / fr
    j = brzina[0] / v_0 # provjeri
    eu = (tlak[0] - tlak[-1]) * v_f / v_0 ** 2
