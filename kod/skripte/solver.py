...
# ne radi
def vanjska_iteracija_stacionarno_velocity_inlet(pretpostavka, brzina_na_ulazu, podrelaksacija):
    tlak = zom_zokg_tlak_stacionarno(pretpostavka[0], pretpostavka[1], p_visokotlacno[0],
                                     p_niskotlacno[0], pretpostavka[2], pretpostavka[3], dx)
    tlak = eksplicitna(tlak, pretpostavka[1], podrelaksacija[1])

    entalpija_na_ulazu = pretpostavka[2, 0]
    gustoca_na_ulazu = pretpostavka[3, 0]

    brzina = zokg_brzina_stacionarno(pretpostavka[0], brzina_na_ulazu, tlak, p_niskotlacno[0], pretpostavka[2],
                         pretpostavka[3], gustoca_na_ulazu, dx, podrelaksacija[0])
...
def rjesavac_stacionarni(pretpostavka, brzina_na_ulazu, podrelaksacija, rezidual_limit):
    ...
    uvjet = True
    while uvjet:
        rjesenje_2 = vanjska_iteracija_stacionarno_velocity_inlet(rjesenje_1, brzina_na_ulazu, podrelaksacija)
        rezidual = manhattan_rjesenja(rjesenje_1, rjesenje_2, 1)
        uvjet = any( np.greater(rezidual, rezidual_limit) )
        rjesenje_1 = rjesenje_2
        broj_iteracija = broj_iteracija + 1
...
podrelaksacije = [0.1, 0.1, 0.1, 0.01, 0.1, 1, 1]
...
# rjesavac_stacionarni(citaj_stanje(137), 14.8, podrelaksacije, reziduali)
